              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                    <div className="bg-black-900/30 p-3 rounded-xl">
                        ...
                        <p className="text-xs text-white flex items-center gap-2"><Clock size={12} className="text-gold-500"/> {formatDateNumericSafe(rental.startDate)}</p>
                    </div>
                    <div className="bg-black-900/30 p-3 rounded-xl">
                        <p className="text-[9px] text-gray-500 uppercase font-bold mb-1">تاريخ التسليم المتوقع</p>
                        <p className="text-xs text-white flex items-center gap-2"><Clock size={12} className="text-red-500"/> {formatDateNumericSafe(rental.expectedEndDate)}</p>
                    </div>
                </div>
                      ...
                      {(rental.fines || []).map((fine) => (
                        <div key={fine.id} className="flex justify-between items-center text-xs bg-black-900/40 p-2 rounded-lg">
                          <div className="text-white">
                            <span className="text-gray-400">{formatDateNumericSafe(fine.date)}</span>
                    ...
                    <div className="space-y-2">
                      {(rental.payments || []).map((pmt) => (
                        <div key={pmt.id} className="text-xs bg-black-900/40 p-2 rounded-lg text-white">
                          <span className="text-gray-400">{formatDateNumericSafe(pmt.date)}</span>
                          <span className="mx-2">•</span>
                       ...
                       <div className="flex justify-between text-sm">
                           ...
                           <span className="text-gold-500 font-bold">
                               {(() => {
                                   ...
                                   return formatDateNumericSafe(d);
                               })()}
                           </span>
                       </div>
                  </div>
